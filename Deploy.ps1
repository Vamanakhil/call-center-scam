    ...
    try {
        ...
        if (-not (Test-Path $profilePath)) {
            ...
            Start-Process 'cmd.exe' -ArgumentList '/c', 'exit' -Credential $cred `
                -WorkingDirectory 'C:\' -WindowStyle Hidden -Wait -ErrorAction SilentlyContinue
...
function Invoke-OneRole {
    ...
    try {
        ...
        $errCount = if ($result.Errors) { $result.Errors.Count } else { 0 }
        if ($errCount -eq 0) {
            Write-OK "$RoleName complete — $($result.FilesCreated) files created"
        } else {
            Write-Warn "$RoleName complete with $errCount warning(s) — $($result.FilesCreated) files created"
            foreach ($e in $result.Errors) { Write-Warn "  · $e" }
        }
        return $result
...
function Write-Summary {
    ...
    foreach ($r in $Results) {
        $fc  = if ($r.FilesCreated) { $r.FilesCreated } else { 0 }
        $ec  = if ($r.Errors)       { $r.Errors.Count  } else { 0 }
        $totalFiles  += $fc
        $totalErrors += $ec
        $status = if ($ec -eq 0) { '✔ DONE' } else { "⚠ $ec warn" }
        $color  = if ($ec -eq 0) { 'Green'  } else { 'Yellow'     }
        Write-Host ("  {0,-14} {1,5} files   {2}" -f $r.Role, $fc, $status) -ForegroundColor $color
    }
    ...
    if ($totalErrors -eq 0) {
        Write-Host "  ✔  All green. Lab machine is ready." -ForegroundColor Green
    } else {
        Write-Host "  ⚠  Setup finished with warnings — review the log." -ForegroundColor Yellow
    }
    Write-Host ""
}
